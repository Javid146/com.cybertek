...
public class IframeExample {
    ...
    public void test() throws InterruptedException {
        ...
        * (1 html inside of other one). So in order to interact we have to switch to iframe: TODO driver.switchTo().frame("id of iframe");
        * //Later if we want to come back to previous html (previous page) we need to type:
        *TODO driver.switchTo().defaultContent();
        *this method also switches to back to previous html (previous page) like the above method TODO driver.switchTo().parentFrame();
        * this method allows you to find frames by index. If you have many frames (many htmls): todo driver.switchTo().frame(0);
